...
class TestDSASigner(unittest.TestCase):
    ...
    def setUp(self):
        self.params, self.x, self.y = DSASigner.generate_params_and_keys()
        self.message = str.encode("Pes Patron", "ascii")

    def test_sign_returns_signature(self):
        signature = DSASigner.sign(self.message, self.params, self.x)
        self.assertIsInstance(signature, DSASignature)

    def test_verify_signature_returns_true(self):
        signature = DSASigner.sign(self.message, self.params, self.x)
        is_valid = DSASigner.verify(self.message, signature, self.params, self.y)
    ...
    def test_verify_signature_returns_false_for_invalid_signature(self):
        ...
        is_valid = DSASigner.verify(self.message, invalid_signature, self.params, self.y)
    ...
    def test_generate_params_and_keys(self):
        params, private_key, public_key = DSASigner.generate_params_and_keys()
